...
class TestBase(object):
    ...
    def destroy_nodes(self, devops_nodes):
        ...
        assert(wait(lambda: self.wait_nodes_get_offline_state(
    ...
    def warm_start_nodes(self, devops_nodes):
        for node in devops_nodes:
            ...
            node.create()
        assert(wait(lambda: self.wait_nodes_get_online_state(),
                    timeout=10 * 60))
    ...
    def wait_nodes_get_offline_state(self, node_names=[]):
        nodes_states = [not x.data['online']
                        for x in self.env.get_all_nodes()
                        if x.data['name'].split('_')[0] in node_names]
        return all(nodes_states)

    def wait_nodes_get_online_state(self):
        return all([node.data['online'] for node in self.env.get_all_nodes()])
