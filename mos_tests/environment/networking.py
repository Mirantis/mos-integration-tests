...
class NetworkCheckBase(object):
    """Class contains common methods for networks checks"""

    @pytest.fixture(autouse=True)
    def init(self, env, os_conn):
        self.env = env
    ...
    def check_vm_connectivity(self, vm_keypair=None, timeout=3 * 60):
        ...
        for server1 in servers:
            ...
            for server2 in servers:
                ...
                ips_to_ping += self.os_conn.get_nova_instance_ips(
                    server2).values()
            self.check_ping_from_vm(server1, vm_keypair,
